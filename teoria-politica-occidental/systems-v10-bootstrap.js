(async()=>{
  ...
  async function runProtected(file){
    try{
      const r=await fetch(`${file}?v=10`,{cache:'no-store'});
  ...
  window.POLIS_V10_STATUS={advancedLoaded:advanced.length-failures.length,failures};
  try{
    await loadScript('systems-learning-v9.js?v=10');
    await loadScript('settings.js?v=10');
  }catch(error){
    failures.push({file:'runtime',message:String(error.message||error)});
    console.error('POLIS v10 runtime failed',error);
  }
  window.dispatchEvent(new CustomEvent('polis:v10Ready',{detail:window.POLIS_V10_STATUS}));
})();
