(async()=>{
  ...
  async function runProtected(file){
    try{
      const r=await fetch(`${file}?v=11`,{cache:'no-store'});
  ...
  window.POLIS_V11_STATUS={modulesLoaded:modules.length-failures.length,failures,systems:window.POLIS_SYSTEMS?.systems?.length||0};
  try{
    await loadScript('systems-learning-v9.js?v=11');
    await loadScript('settings.js?v=11');
  }catch(error){
    failures.push({file:'runtime',message:String(error.message||error)});
    console.error('POLIS v11 runtime failed',error);
  }
  window.dispatchEvent(new CustomEvent('polis:v11Ready',{detail:window.POLIS_V11_STATUS}));
})();
